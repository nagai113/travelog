...
function ci_comment_form( $args = array(), $post_id = null ) {
	...
	$fields =  array(
		'author' => '<fieldset><label for="author">' . __('Name', 'ci_theme') . ': '.$req_string.'</label><input type="text" class="text input-text" id="author" name="author" value="' . esc_attr( $commenter['comment_author'] ) . '"' . $aria_req . ' /></fieldset>',
		'email'  => '<fieldset><label for="email">' . __('Email', 'ci_theme') . ': '.$req_string.'</label><input type="text" class="text input-text" id="email" name="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '"' . $aria_req . ' /></fieldset>',
		'url'    => '<fieldset><label for="url">' . __('Website', 'ci_theme') . ':</label><input type="text" class="text input-text" id="url" name="url" value="' . esc_attr( $commenter['comment_author_url'] ) . '" /></fieldset>',
	);

	$required_text = sprintf( ' ' . __('Required fields are marked %s', 'ci_theme'), '<span class="required">*</span>' );
	$defaults = array(
		'fields'               => apply_filters( 'comment_form_default_fields', $fields ),
		'comment_field'        => '<fieldset><label for="comment">' . _x( 'Comment', 'noun', 'ci_theme' ) . ':</label><textarea cols="5" rows="5" id="comment" name="comment" aria-required="true"></textarea></fieldset>',
