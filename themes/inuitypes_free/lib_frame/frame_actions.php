...
function bizz_wp_pagenavi_init() {
    ...
    $pagenavi_options['first_text'] = '&laquo; '.__('First', 'bizzthemes');
    $pagenavi_options['last_text'] = __('Last', 'bizzthemes').' &raquo;';
		...
		<div class="text-right">
			...
			<div class="comm-reply <?php if (1 == $comment->user_id) echo "authcomment"; ?>">
			    ...
			    <span class="fr">
					...
					?>
					<?php comment_reply_link(array_merge( $args, array('reply_text' => __('Reply', 'bizzthemes'), 'depth' => $depth, 'max_depth' => $args['max_depth']))) ?>
				</span>
			</div><!-- /.comm-reply -->
...
function hosted_twitter_script( $twitid = '', $username = '', $count=1 ) {
	...
	$before_timesince = do_action( 'beforetimesince' );
...
<?php 
			...
			if ( isset($GLOBALS['opt']['bizzthemes_archive_full']) ) {
				the_content(stripslashes($GLOBALS['opt']['bizzthemes_readmore_text']));
			} else {
				the_excerpt();
